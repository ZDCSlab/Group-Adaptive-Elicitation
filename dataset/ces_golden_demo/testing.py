...
def community_neighbor_occurrence(stats_dict, topk=10):
    """
    stats_dict: {
        caseid(str): {
            'caseid': str,
            'community': str or int,
            'neighbors': List[str]   # caseids
        }, ...
    }
    ...
    Returns:
      result: {
        community_id: [
          {'caseid': str, 'occurrence': int, 'community': str, 'ratio': float},
          ...  # sorted by occurrence desc, then caseid
        ],
        ...
      }
    """
    # 1) Build community -> members
    comm2members = defaultdict(list)
    for cid, rec in stats_dict.items():
        comm2members[str(rec['community'])].append(cid)

    # 2) Count, per community, how often a member appears in the neighbors of other members (same community)
    result = {}
    for comm, members in comm2members.items():
        members_set = set(members)
        counts = Counter({m: 0 for m in members})  # ensure every member appears even if 0

        # For each member u in this community, look at its neighbors and count those that are also in the community
        for u in members:
            neighs = stats_dict[u].get('neighbors', [])[:topk] or []
            for v in neighs:
                if v != u and v in members_set:
                    counts[v] += 1
            ...
            (
                {
                    ...
                    'occurrence': counts[m],
                    'community': comm,
                    'ratio': counts[m] / comm_size if comm_size > 0 else 0.0,
                }
                for m in members
            ),
            key=lambda d: (-d['occurrence'], d['caseid'])
        )
        result[comm] = ranked

    return result

def rank_nodes_by_ratio(neighbors_info, per_community=False):
    """
    If per_community=True, return {community: [sorted members by ratio]}
    Else return one global list sorted by ratio desc, then occurrence desc, then caseid.
    """
    occ = community_neighbor_occurrence(neighbors_info)

    if per_community:
        return {
            comm: sorted(rows, key=lambda d: (-d['ratio'], -d['occurrence'], d['caseid']))
            for comm, rows in occ.items()
        }

    # Flatten to a single list and sort
    rows = [row for rows in occ.values() for row in rows]
    rows.sort(key=lambda d: (-d['ratio'], -d['occurrence'], d['caseid']))
    return rows

import math
from collections import Counter

def top_decile_by_ratio(neighbors_info, percentile=0.10):
    """
    Returns:
      top_list:  list[dict]  # top p% nodes sorted by ratio desc (then occurrence, caseid)
      dist:      dict        # {community: {"count": c, "share": c/len(top_list)}}
    """
    # 1) per-community occurrences + ratio
    occ = community_neighbor_occurrence(neighbors_info)  # assumes your earlier function
    ...
    # 3) take top p%
    k = max(1, math.ceil(percentile * len(rows)))
    top_list = rows[:k]

    # 4) community distribution within the top set
    comm_counts = Counter(r['community'] for r in top_list)
    total = len(top_list)
    dist = {c: {"count": cnt, "share": cnt / total} for c, cnt in sorted(comm_counts.items())}

    return top_list, dist


# Load communities of testing set from /home/ruomeng/gae/dataset/ces_golden_demo/raw/24/neighbors_24.jsonl
year = '24'
jsonl_file = f"/home/ruomeng/gae/dataset/ces_golden_demo/raw/{year}/neighbors_noisy_{year}.jsonl"  
neighbors_info = load_jsonl_as_dict_of_dict(jsonl_file, key='caseid')
occur = community_neighbor_occurrence(neighbors_info, topk=10)

import math

def top_decile_caseids_each_community(ranked_per_comm, percentile=0.10, min_per_comm=1, dedup=True):
    """
    ranked_per_comm: {community_id: [ {'caseid', 'ratio', 'occurrence', ...}, ... ]} 
                     (sorted desc by ratio already)
    Returns a flat list of caseids: top p% from each community.
    """
    caseids = []
    # Use sorted keys for deterministic order; remove "sorted(...)" if you prefer dict insertion order
    for comm in sorted(ranked_per_comm.keys()):
        rows = ranked_per_comm[comm]
        if not rows:
            continue
        k = max(min_per_comm, math.ceil(percentile * len(rows)))
        top_rows = rows[:k]
        caseids.extend(r['caseid'] for r in top_rows)

    if dedup:
        # preserve first appearance order
        caseids = list(dict.fromkeys(caseids))
    return caseids

# Usage with your object:
ranked_global = rank_nodes_by_ratio(neighbors_info, per_community=True)
# caseid_lst = []
# for item in ranked_global:
#     caseid_lst.append(item['caseid'])
# top_caseids = caseid_lst[:int(len(caseid_lst)*0.1)]
# pd.DataFrame({'caseid': top_caseids}).to_csv('/home/ruomeng/gae/dataset/ces_golden_demo/raw/24/top_caseids.csv', index=False)

top_caseids = top_decile_caseids_each_community(ranked_global, percentile=0.10)
print(len(top_caseids), top_caseids[:5])

import pandas as pd

pd.DataFrame({'caseid': top_caseids}).to_csv('/home/ruomeng/gae/dataset/ces_golden_demo/raw/24/top_caseids_top5.csv', index=False)

...
community_lst = pd.read_csv('/home/ruomeng/gae/dataset/ces_golden_demo/raw/24/community_test.csv')['community'].tolist()
com_dict = {c: [] for c in community_lst}

# Populate
for rec in neighbors_info.values():
    # Defensive gets + normalization
    cid = rec.get("caseid")
    com = rec.get("community")
    if com in community_lst:
        com_dict[com].append(cid)
...
# Add noise by exchange nodes among different communities
# com_dict: {"C1": ["u1","u2","u3",...], "C2": [...], ...}
noisy_com_dict, moves = exchange_noise(com_dict, frac=0.5, min_per_comm=1, seed=42)

dict_features = df_to_feature_map(df_feature, id_col="caseid")

# neighbors_same_noisy = neighbors_by_hamming_from_com(noisy_com_dict, dict_features, K=20, min_overlap=0.7)


neighbors_same_noisy = neighbors_by_hamming_from_com(
    noisy_com_dict,
    dict_features,
    K=15,
    min_similarity=0.7,  # 至少 70% 维度匹配
    min_compared=1       # 至少 1 个可比维度（可按需提高，比如 10/15）
)
print(neighbors_same_noisy)
for node in neighbors_same_noisy:
    if len(neighbors_same_noisy[node]) == 0:
        neighbors_same_noisy[node] = [node]

# neighbors_same_noisy["1235066043"] = ["1235066043"]
# {"caseid": "1235066043", "community": "71", "neighbors": []}
# Invert noisy_com_dict to {caseid -> community}
caseid2com = {str(cid): str(com)
              for com, ids in noisy_com_dict.items()
              for cid in ids}
...
out_path = "/home/ruomeng/gae/dataset/ces_golden_demo/raw/24/neighbors_noisy50_24.jsonl"  # <- change if needed
...
# per-community and global disagreement (after noise or before—your choice)
res = summarize_intra_disagreement(
    com_dict=noisy_com_dict,          # or original com_dict
    dict_features=dict_features,
    min_overlap=1,                    # raise for stricter comparisons
    micro_weighted_majority=False,    # True to weight features by #valid rows
)
summary_path = "/home/ruomeng/gae/dataset/ces_golden_demo/raw/24/intra_disagreement_summary_noisy50_com_dict.json"
...
# per-community and global disagreement (after noise or before—your choice)
res = summarize_intra_disagreement(
    com_dict=com_dict,          # or original com_dict
    dict_features=dict_features,
    min_overlap=1,                    # raise for stricter comparisons
    micro_weighted_majority=False,    # True to weight features by #valid rows
)
summary_path = "/home/ruomeng/gae/dataset/ces_golden_demo/raw/24/intra_disagreement_summary_com_dict.json"
with open(summary_path, "w", encoding="utf-8") as f:
    json.dump(res, f, ensure_ascii=False, indent=2)
print("Wrote:", summary_path)

...
def run_group_adaptive_elicitation(
    ...
    for t in range(T):
        ...
        # query_lst = ["333d","327b", "332f", "334h", "327d"]
        # query_lst = ["332g", "331a", "334g",  "355e", "332a"]
        # x_star = [query_lst[t]]
        if 'random' in mode:
        ...
        if not MIG and k_nodes == len(dataset.graph.nodes):
            ...
            migs_count = 0
        elif node_select == 'occurence_out':
...
def parse_args():
    ...
    p.add_argument("--node_select", default="random", choices=["random", "entropy", "random_perc", "entropy_perc", "top_caseids_top5", "occurence_out"])
...
if __name__ == "__main__":
    ...
    jsonl_file = f"/home/ruomeng/gae/dataset/ces_golden_demo/raw/{year}/neighbors_demo_{year}.jsonl"  
    ...
    if args.MIG:
        save_path = f'./results_tuning_demo/results_{mode}/{year}_{mode}_node_select_{node_select}_MIG_T{T}_{x_heldout}_{args.imputation}.jsonl'

    else:
        save_path = f'./results_tuning_demo/results_{mode}/{year}_{mode}_node_select_{node_select}_N{args.N}_percent{selected_respodent}_T{T}_{x_heldout}_{args.imputation}_top_k_nei{args.top_k_nei}.jsonl'

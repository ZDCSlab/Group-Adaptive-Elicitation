...
def sampling(
    ...
    probs_batch_node_rest = pool.predict("iid", items=dataset.graph.nodes, shard_arg="nodes", query=dataset.codebook[X]["question"], asked_queries=dataset.asked_queries,
                                    neighbors=dataset.graph.neighbor, observed=dataset.observed_dict, estimated=None, mode="iid")
    ...
    # sampling
    traj = batched_updates(y_init, observed, dataset, pool, X, mode=mode, max_iter=N, verbose=verbose)

...
def get_neighbor_ids(entry):
        for k in ['neighbors', 'neighbor_ids', 'topk', 'top30', 'ids', 'topk_ids']:
...
def compute_neighbor_stats_A(nei_df, question_id, codebook=None):
    ...
    summary_text = (f"Majority Answer: {optoins[int(maj_key)-1]}{maj_meaning}")
...
def load_ces_dataset_partition(year, split='train'):
    out_dir = Path(f'/home/ruomeng/gae/dataset/ces_promaxmax/processed')
    out_dir.mkdir(parents=True, exist_ok=True)  
    out_path = Path(f'{out_dir}/ces_{year}_{split}.jsonl')
    ...
    jsonl_file = f"/home/ruomeng/gae/dataset/ces_promaxmax/raw/{year}/neighbors_{year}.jsonl"  
    neighbors_info = load_jsonl_as_dict_of_dict(jsonl_file, key='caseid')
    
    df_survey = pd.read_csv(f"/home/ruomeng/gae/dataset/ces_promaxmax/raw/{year}/questions_{split}_{year}.csv")
    caseid_lst = list(df_survey["caseid"])
    ...
    for respondent in tqdm(caseid_lst, total=len(caseid_lst), desc="Building dataset", dynamic_ncols=True):
        ...
        for question_id, val in respondent_data.items():
            ...
            q_text = f'{codebook[question_id]["question"]}\nA. Support\nB. Oppose'
            ...
            options = ["A", "B"]
            if int(val) == -1:
                a_text = int(summary_text["majority_answer"])
                print('question_id', a_text)
            else:
                a_text = int(val) # f'[{int(val)}] {codebook[question_id]["options"][str(int(val))]}'

            respondent_designs.append({"q_id": question_id, "question": q_text, "answer": options[a_text-1], "neighbor_stats": summary_text['summary_text']})
        
        dataset.append({'case_id': respondent, 'survey': respondent_designs})
...
if __name__ == "__main__":
    ...
    for include_neighbor in [True, False]:
    
        for year in ['24']:
            data_dict = dict()
            for split in ['train', 'val', 'test']:
                dataset = load_ces_dataset_partition(year=year, split=split)
                dataset_str = train_split_entities_from_json(dataset, include_neighbor=include_neighbor)
                data_dict[split] = dataset_str

            save_dir = Path("/home/ruomeng/gae/dataset/ces_promaxmax/processed")
            save_dir.mkdir(parents=True, exist_ok=True)
            file_name = f"dataset_{year}_neighbor{int(include_neighbor)}.json"
